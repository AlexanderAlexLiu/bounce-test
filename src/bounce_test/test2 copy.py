...
class Game:
    ...
    def setup(self) -> None:
        self.lines = ((450, 400), (450, 250)), ((350, 200), (350, 350)), ((200, 200), (600, 200)), ((200, 200), (200, 400)), ((600, 200), (600, 400)), ((200, 400), (600, 400))
    ...
    def get_points(self, n, start, end, last_line):
        ...
        for line in self.sort_list(start):
            ...
            if inter_x_y != None and line != last_line:
                ...
                break
        if inter_x_y == None:
            pg.draw.line(self.surface, Color.GREEN, start, end)
        elif n == 0:
            pg.draw.line(self.surface, Color.GREY, start, inter_x_y)
        else:
            refl_point = self.return_refl(end, inter_x_y, last_line)
            pg.draw.line(self.surface, Color.RED, start, inter_x_y)
    ...
    def calc_Ldist(self, a, point):
        ...
        t = max(0, min(1, self.dot((point[0]-a[0][0], point[1]-a[0][1]), (a[0][0]-a[1][0], a[0][1]-a[1][1]))))
    ...
    def run(self) -> None:
        ...
        while 1:
            ...
            for line in self.lines:
                pg.draw.line(self.surface, Color.BLACK, line[0], line[1])
            a = (275, 300)
            b = (self.mouse_x, self.mouse_y)
            d = pg.Vector2(b[0]-a[0], b[1]-a[1]) * 10
            c = int(d.x+a[0]), int(d.y+a[1])
            self.get_points(100, a, c, None)
            pg.display.flip()
